...
typedef struct OperatorMapping
{
	char	   *ompname;
	Oid			ompoprid;
	char	   *ompformat;
...
/* operator mapping */
static const OperatorMapping operator_mapping[] = {
	/* LIKE operator */
	{"like", 1209, "%s LIKE %s ESCAPE '\\'"},
	{"like", 1211, "%s LIKE %s ESCAPE '\\'"},
	/* NOT LIKE operator */
	{"not like", 1210, "%s NOT LIKE %s ESCAPE '\\'"},
	{"not like", 1212, "%s NOT LIKE %s ESCAPE '\\'"},
...
static const OperatorMapping *get_operator_mapping(Oid opno);
static StringInfo * get_rule_expr_parens(List *args, deparse_context *context,
						Node *expr);
static void ora_get_func_expr(FuncExpr *expr, deparse_context *context,
				bool showimplicit, Oid foreigntableid);
	...
	if (!attrsOnly)
	{
		if (!isConstraint)
			appendStringInfo(&buf, "CREATE %sINDEX %s ON %s USING %s (",
							 ...
							 ora_quote_identifier(NameStr(idxrelrec->relname)),
							 generate_relation_name(indrelid, NIL),
							 ora_quote_identifier(NameStr(amrec->amname)));
		...
		if (showTblSpc)
		{
			...
			if (OidIsValid(tblspc))
			{
				...
				appendStringInfo(&buf, " TABLESPACE %s",
							  ora_quote_identifier(get_tablespace_name(tblspc)));
			}
		}
							...
							int prettyFlags)
{
	...
	if (fullCommand && OidIsValid(conForm->conrelid))
	{
		appendStringInfo(&buf, "ALTER TABLE ONLY %s ADD CONSTRAINT %s ",
						 ...
						 ora_quote_identifier(NameStr(conForm->conname)));
	}
							 ...
							 StringInfo buf)
{
	...
	for (j = 0; j < nKeys; j++)
	{
		...
		if (j == 0)
			appendStringInfoString(buf, ora_quote_identifier(colName));
		else
			appendStringInfo(buf, ", %s", ora_quote_identifier(colName));
	}
}
...
pg_get_functiondef(PG_FUNCTION_ARGS)
{
	...
	appendStringInfo(&buf, "\n LANGUAGE %s\n",
					 ora_quote_identifier(NameStr(lang->lanname)));
			...
			if (!isnull)
			{
				...
				*pos++ = '\0';
				...
				appendStringInfo(&buf, " SET %s TO ",
								 ora_quote_identifier(configitem));
						 ...
						 bool print_table_args, bool print_defaults)
{
	...
	for (i = 0; i < numargs; i++)
	{
		...
		if (argname && argname[0])
			appendStringInfo(buf, "%s ", ora_quote_identifier(argname));
...
/*
 * ora_deparse_expression			- General utility for deparsing expressions
 *
 * calls deparse_expression_pretty with all prettyPrinting disabled
 */
char *
ora_deparse_expression(Node *expr, List *dpcontext,
				   bool forceprefix, bool showimplicit)
{
			 ...
			 int prettyFlags)
{
	...
	if (ev_attr > 0)
		appendStringInfo(buf, ".%s",
						 ora_quote_identifier(get_relid_attribute_name(ev_class,
																   ev_attr)));
...
get_with_clause(Query *query, deparse_context *context)
{
	...
	foreach(l, query->cteList)
	{
		...
		appendStringInfoString(buf, sep);
		appendStringInfoString(buf, ora_quote_identifier(cte->ctename));
		if (cte->aliascolnames)
		{
			...
			foreach(col, cte->aliascolnames)
			{
				if (first)
					...
					appendStringInfoString(buf, ", ");
				appendStringInfoString(buf,
									   ora_quote_identifier(strVal(lfirst(col))));
			}
			appendStringInfoChar(buf, ')');
		}
		appendStringInfoString(buf, " AS (");
	...
	if (query->hasForUpdate)
	{
		foreach(l, query->rowMarks)
		{
			...
			appendStringInfo(buf, " OF %s",
							 ora_quote_identifier(rte->eref->aliasname));
			if (rc->noWait)
	...
	foreach(l, targetList)
	{
		...
		if (colname)			/* resname could be NULL */
		{
			if (attname == NULL || strcmp(attname, colname) != 0)
				appendStringInfo(buf, " AS %s", ora_quote_identifier(colname));
		}
	}
}
...
get_rule_windowclause(Query *query, deparse_context *context)
{
	...
	foreach(l, query->windowClause)
	{
		...
		appendStringInfo(buf, "%s AS ", ora_quote_identifier(wc->name));
					...
					deparse_context *context)
{
	...
	if (wc->refname)
	{
		appendStringInfoString(buf, ora_quote_identifier(wc->refname));
		needspace = true;
	}
...
get_insert_query_def(Query *query, deparse_context *context)
{
	...
	foreach(l, query->targetList)
	{
		 ...
		 */
		appendStringInfoString(buf,
						ora_quote_identifier(get_relid_attribute_name(rte->relid,
															   tle->resno)));
...
get_update_query_def(Query *query, deparse_context *context)
{
	...
	if (rte->alias != NULL)
		appendStringInfo(buf, " %s",
						 ora_quote_identifier(rte->alias->aliasname));
...
get_utility_query_def(Query *query, deparse_context *context)
{
	...
	if (query->utilityStmt && IsA(query->utilityStmt, NotifyStmt))
	{
		...
		appendStringInfo(buf, "NOTIFY %s",
						 ora_quote_identifier(stmt->conditionname));
		if (stmt->payload)
		{
			appendStringInfoString(buf, ", ");
...
get_variable(Var *var, int levelsup, bool showstar, deparse_context *context)
{
	...
	if (refname && (context->varprefix || attname == NULL))
	{
		if (schemaname)
			appendStringInfo(buf, "%s.",
							 ora_quote_identifier(schemaname));
		appendStringInfoString(buf, ora_quote_identifier(refname));
		if (attname || showstar)
			appendStringInfoChar(buf, '.');
	}
	if (attname)
			  ...
			  bool showimplicit)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_Const:
			get_const_expr((Const *) node, context, -1);
			break;
		...
		case T_NamedArgExpr:
			{
				...
				appendStringInfo(buf, "%s := ", ora_quote_identifier(na->name));
		...
		case T_FieldSelect:
			{
				...
				fieldname = get_name_for_var_field((Var *) arg, fno,
												   0, context);
				appendStringInfo(buf, ".%s", ora_quote_identifier(fieldname));
			}
			break;
		...
		case T_XmlExpr:
			{
				...
				if (xexpr->name)
				{
					appendStringInfo(buf, "NAME %s",
									 ora_quote_identifier(map_xml_name_to_sql_identifier(xexpr->name)));
					needcomma = true;
				}
				if (xexpr->named_args)
				{
					...
					forboth(arg, xexpr->named_args, narg, xexpr->arg_names)
					{
						...
						appendStringInfo(buf, " AS %s",
										 ora_quote_identifier(map_xml_name_to_sql_identifier(argname)));
						needcomma = true;
					}
					if (xexpr->op != IS_XMLFOREST)
						appendStringInfoChar(buf, ')');
		...
		case T_CurrentOfExpr:
			{
				...
				if (cexpr->cursor_name)
					appendStringInfo(buf, "CURRENT OF %s",
									 ora_quote_identifier(cexpr->cursor_name));
				else
...
get_oper_expr(OpExpr *expr, deparse_context *context)
{
	...
	Oid			opno = expr->opno;
	List	   *args = expr->args;
	const OperatorMapping  *map;

	if (!PRETTY_PAREN(context))
		appendStringInfoChar(buf, '(');

	if ((map = get_operator_mapping(opno)) != NULL)
	{
		StringInfo *bufs;

		bufs = get_rule_expr_parens(args, context, (Node *) expr);

		if (list_length(args) == 1)
		{
			appendStringInfo(buf, map->ompformat, bufs[0]->data);
		}
		else
		{
			appendStringInfo(buf, map->ompformat, bufs[0]->data, bufs[1]->data);
		}
	}
			  ...
			  bool showimplicit)
{
	...
	foreach(l, context->namespaces)
	{
		...
		foreach(rtlist, dpns->rtable)
		{
			...
			if (strcmp(rte->eref->aliasname, "oracle_fdw_foreigntable") == 0)
			{
				ora_get_func_expr(expr, context, showimplicit, rte->relid);
				return;
			}
		}
	}
...
get_windowfunc_expr(WindowFunc *wfunc, deparse_context *context)
{
	...
	foreach(l, context->windowClause)
	{
		...
		{
			if (wc->name)
				appendStringInfoString(buf, ora_quote_identifier(wc->name));
			else
...
get_const_expr(Const *constval, deparse_context *context, int showtype)
{
	...
	switch (constval->consttype)
	{
		case FLOAT4OID:
			{
				float4	num = DatumGetFloat4(constval->constvalue);

				if (isnan(num))
					appendStringInfo(buf, "binary_float_nan");
				else if (is_infinite(num) == 1)
					appendStringInfo(buf, "binary_float_infinity");
				else if (is_infinite(num) == -1)
					appendStringInfo(buf, "-binary_float_infinity");
				else
					break;
		...
		case FLOAT8OID:
			{
				float8	num = DatumGetFloat8(constval->constvalue);

				if (isnan(num))
					appendStringInfo(buf, "binary_double_nan");
				else if (is_infinite(num) == 1)
					appendStringInfo(buf, "binary_double_infinity");
				else if (is_infinite(num) == -1)
					appendStringInfo(buf, "-binary_double_infinity");
				else
					break;
		...
		case BYTEAOID:
			deparseBytea(buf, constval->constvalue);
			return;

		case INTERVALOID:
			deparseInterval(buf, constval->constvalue);
			return;

		case DATEOID:
			deparseDate(buf, constval->constvalue);
			return;

		case TIMESTAMPOID:
			deparseTimestamp(buf, constval->constvalue);
			return;

		case TIMESTAMPTZOID:
			deparseTimestamptz(buf, constval->constvalue);
			return;
	...
	if (constval->consttype == NUMERICOID && strcmp(extval, "NaN") == 0)
	{
		appendStringInfo(buf, "binary_double_nan");
		return;
	}
...
/*
 * In oracle , the escape is good only in single quotes
 */
#undef SQL_STR_DOUBLE
#define SQL_STR_DOUBLE(ch, escape_backslash)	((ch) == '\'')
...
get_from_clause_item(Node *jtnode, Query *query, deparse_context *context)
{
	...
	if (IsA(jtnode, RangeTblRef))
	{
		...
		switch (rte->rtekind)
		{
			...
			case RTE_VALUES:
				...
				break;
			case RTE_CTE:
				appendStringInfoString(buf, ora_quote_identifier(rte->ctename));
				break;
			default:
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		{
			appendStringInfo(buf, " %s",
							 ora_quote_identifier(j->alias->aliasname));
				 ...
				 StringInfo buf)
{
		...
		GetDefaultOpClass(actual_datatype, opcrec->opcmethod) != opclass)
	{
		...
		if (OpclassIsVisible(opclass))
			appendStringInfo(buf, " %s", ora_quote_identifier(opcname));
		else
		{
			nspname = get_namespace_name(opcrec->opcnamespace);
			appendStringInfo(buf, " %s.%s",
							 ora_quote_identifier(nspname),
							 ora_quote_identifier(opcname));
		}
	}
...
/*
 * ora_quote_identifier			- Quote an identifier only if needed
 *
 * We malloc the required space; slightly space-wasteful but well worth it for
 * notational simplicity.
 */
const char *
ora_quote_identifier(const char *ident)
{
	/*
	 * Can avoid quoting if ident starts with a uppercase letter and and
	 * contains only uppercase letters, digits, underscores, dollar sign and
	 * pound sign, *and* is not any SQL keyword.  Otherwise, supply quotes.
	 */
	int		len;
	char   *result;

	/* identifier must be from 1 to 30 bytes long in the Oracle */
	len = strlen(ident);
	if (len > 30)
		ereport(ERROR,
				(errcode(ERRCODE_FDW_INVALID_DESCRIPTOR_FIELD_IDENTIFIER),
				 errmsg("identifier is too long: \"%s\"", ident)));

	if (strchr(ident, '"') != NULL)
		ereport(ERROR,
				(errcode(ERRCODE_FDW_INVALID_DESCRIPTOR_FIELD_IDENTIFIER),
				 errmsg("identifier can't contain double quotation marks: \"%s\"", ident)));

	result = (char *) palloc(len + 2 + 1);
	snprintf(result, len + 2 + 1, "\"%s\"", ident);
						   ...
						   const char *ident)
{
	...
	initStringInfo(&buf);
	if (qualifier)
		appendStringInfo(&buf, "%s.", ora_quote_identifier(qualifier));
	appendStringInfoString(&buf, ora_quote_identifier(ident));
	return buf.data;
}
...
generate_operator_name(Oid operid, Oid arg1, Oid arg2)
{
	...
	else
	{
		...
		appendStringInfo(&buf, "OPERATOR(%s.", ora_quote_identifier(nspname));
	}
 ...
 */
static const OperatorMapping *
get_operator_mapping(Oid opno)
{
	int	i;

	for (i = 0; operator_mapping[i].ompname != NULL; i++)
	{
		if (opno == operator_mapping[i].ompoprid)
			return operator_mapping + i;
	}
...
static StringInfo *
get_rule_expr_parens(List *args, deparse_context *context, Node *expr)
{
	int					i;
	int					len;
	StringInfo		   *bufs;
	...
	bufs = (StringInfo *) palloc(len * sizeof(StringInfo));

	ctx.namespaces = context->namespaces;
	ctx.windowClause = context->windowClause;
	ctx.windowTList = context->windowTList;
	ctx.prettyFlags = context->prettyFlags;
	ctx.indentLevel = context->indentLevel;
	ctx.varprefix = context->varprefix;
	...
	foreach (l, args)
	{
		...
		ctx.buf = bufs[i] = makeStringInfo();
		get_rule_expr_paren(arg, &ctx, true, expr);

		/*
		 * In Oracle, only underscore, percent sign or escape character is
		...
		if (nodeTag(arg) == T_Const)
		{
			...
			dest = src = bufs[i]->data;

			while (*src != '\0')
			{
				if (*src == '\\')
				{
					if (src[1] != '_' && src[1] != '%' && src[1] != '\\')
					{
						/* skip an unjust escape */
						src++;
					}
					else
					{
...
/*
 * ora_get_func_expr			- Parse back a FuncExpr node
 */
static void
ora_get_func_expr(FuncExpr *expr, deparse_context *context,
			  bool showimplicit, Oid foreigntableid)
{
	ForeignTable   *table;
	RoutineMapping *rm;
	...
	rm = GetRoutineMapping(expr->funcid, table->serverid);
	format = GetFdwOptionList(rm->options, "format");

	/* format function call */
	cn = makeNode(Const);
	cn->consttype = CSTRINGOID;
	fn = makeNode(FuncExpr);
	fn->args = lappend(fn->args, cn);
	flinfo.fn_expr = (Node *) fn;
	fcinfo.arg[0] = CStringGetTextDatum(format);
	...
	foreach(l, expr->args)
	{
		...
		fn->args = lappend(fn->args, cn);
		fcinfo.arg[i] = CStringGetDatum(
			ora_deparse_expression(arg, context->namespaces, false, false));
		fcinfo.argnull[i] = false;
		i++;
	}

	InitFunctionCallInfoData(fcinfo, &flinfo, i, InvalidOid, NULL, NULL);

	result = TextDatumGetCString(text_format(&fcinfo));
	elog(DEBUG1, "push down function is: %s", result);
